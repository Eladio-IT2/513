...
$orders = array_slice(get_all_orders(), 0, 15);
$users = array_slice(get_all_users(), 0, 10);
...
<section class="admin-content">
    ...
    <?php foreach ($orders as $order): ?>
                        <tr>
                            <td>#<?php echo (int) ($order['order_id'] ?? $order['id'] ?? 0); ?></td>
                            <td><?php echo htmlspecialchars($order['full_name'] ?? ''); ?> <br><small><?php echo htmlspecialchars($order['customer_email'] ?? ($order['email'] ?? '')); ?></small></td>
                            <td>$<?php echo number_format((float) ($order['total_amount'] ?? 0), 2); ?></td>
                            <td><span class="status-badge status-<?php echo htmlspecialchars($order['status'] ?? 'pending'); ?>"><?php echo ucfirst($order['status'] ?? 'pending'); ?></span></td>
                            <td><?php echo date('M j, Y', strtotime($order['order_date'] ?? ($order['created_at'] ?? 'now'))); ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
